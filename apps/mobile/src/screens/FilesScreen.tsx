...
export function FilesScreen() {
  ...
  return (
    <Screen title="Files" statusText={statusText} scroll={false}>
      <View className="flex-1">
        <Card>
          ...
          <View className="mt-2">
            {items.length ? (
              items.map((f, idx) => (
                <ListRow key={f.id} title={f.name || 'Unnamed file'} rightText={f.id.slice(0, 6)} showDivider={idx !== items.length - 1} />
              ))
            ) : (
              <Text className="text-sm opacity-70 py-6">No files yet.</Text>
            )}
          </View>
